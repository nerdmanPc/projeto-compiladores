...
int main () {
  ...
  int lin = 1, col=0;
   ...
   for(int k =0;k<buffer.size();k++){
      int ascll = buffer[k];
         if(buffer[k]!=' '&&((ascll>=65&&ascll<=90)||(ascll>=97&&ascll<=122)||(ascll>=48&&ascll<=57))) // buffer[k] é uma letra ou numero válido (tabela ascll)
         {
             if(ascll==35)//comentario
             coment =1;

             if(first==' '&& coment==0) //se não salvei nada no primeiro caracter ainda
             {
             ...
             else if(coment==0&&first!=' '&&((ascll_first>=65&&ascll_first<=90)||(ascll_first>=97&&ascll_first<=122))){ // first é uma letra, logo todo char valido continua valido
                aux_palavra  = aux_palavra +buffer[k]; //armazena na palavra a ser gerada
             }
             else if(coment==0&&first!=' '&&((ascll_first>=48&&ascll_first<=57)&&(ascll>=48&&ascll<=57))){// first é um numero, portanto c deve ser um numero
                aux_palavra  = aux_palavra +buffer[k]; //armazena na palavra a ser gerada
             }
             else{
                          ...
                          else if(guarda.top()=="esquerda"||guarda.top()=="direita")
                          {
                               if(aux_palavra=="robo")
                              {
                                 ...
                                 if(buffer[k]!='\n'&&k<buffer.size()-1){ //da para continuar
                                    ...
                                    guarda.push(aux_palavra);  //não da para alocar outra palavra na sequencia
                                     while(guarda.size()>0)
                                    {
                                        inverte.push(guarda.top());
                                        guarda.pop();
                ...
                else if(estado==2&&(aux_palavra=="a"||aux_palavra=="bloqueada"))
                {
                 ...
                 else if(estado==3&&(aux_palavra=="direita"||aux_palavra=="esquerda"||aux_palavra=="frente")) //só tem uma forma de chegar aqui
                 {
                    ...
                    if(estado==0){ //id
                        ...
                        while(inverte.size()>0){ //imprimir o primeiro da sequencia que não deu certo como id/ exceto esquerda e direita
                                    if(inverte.size()>1){
                                        guarda.push(inverte.front());
                                        inverte.pop();
                                    }
